...
; (function ($, w, undefined) {
    ...
    function Sly(frame, options, callbackMap) {
        ...
        var $wheel = o.wheel ? $(o.wheel).eq(0) : $frame.children().eq(0);
        var frameSize = 0;
        var slideeSize = 0;
        var pos = {
            start: 0,
        ...
        var wheelStyles = new StyleRestorer($wheel[0]);
        ...
        function slideTo(newPos, immediate, dontAlign) {
            ...
            var $wheelContainer = $(".wheel-outer");
            var $wheelNav = $wheelContainer.find(".plus-wrap");

            $wheelContainer.removeClass("w-1").removeClass("w-2").removeClass("w-2-2").removeClass("w-3").removeClass("w-3-2").removeClass("w-4").removeClass("w-4-2");
            $wheelNav.children("button").removeClass("active").attr("aria-selected", false);
            var wheelData = $("#wheel-co-wrap").find('.active').data("wheel-item");
            $wheelNav.children("button[data-wheel-item = " + wheelData + "]").addClass('active').attr("aria-selected", true);
            $wheelNav.children("button[data-wheel-item = " + wheelData + "]").parent("div").parent("nav").parent("div").parent("div").parent("div").parent($wheelContainer).addClass(wheelData);

            $(".close").attr("tabindex", "-1");
            $(".w-2").find(".items").children("li[data-wheel-item='w-2']").children(".wheel-box").children(".close").attr("tabindex", "0");
            $(".w-3").find(".items").children("li[data-wheel-item='w-3']").children(".wheel-box").children(".close").attr("tabindex", "0");
            $(".w-4").find(".items").children("li[data-wheel-item='w-4']").children(".wheel-box").children(".close").attr("tabindex", "0");

            $(".btn-cta").attr("tabindex", "-1");
            $(".w-2").find(".items").children("li[data-wheel-item='w-2']").children(".wheel-box").children(".btn-cta").attr("tabindex", "0");
            $(".w-3").find(".items").children("li[data-wheel-item='w-3']").children(".wheel-box").children(".btn-cta").attr("tabindex", "0");
            $(".w-4").find(".items").children("li[data-wheel-item='w-4']").children(".wheel-box").children(".btn-cta").attr("tabindex", "0");

            var $bodyOuter = $("body");
            $bodyOuter.removeClass("wheel-active");
            if (wheelData == "w-2" || wheelData == "w-2-2" || wheelData == "w-3" || wheelData == "w-3-2" || wheelData == "w-4" || wheelData == "w-4-2") {
                $bodyOuter.addClass("wheel-active");
            } else {
                $bodyOuter.removeClass("wheel-active");
            }
            if ($(window).width() > 839) {
                if ($(".wheel-outer.w-2").length) {
                    $('html,body').animate({ scrollTop: $($wheelContainer).offset().top }, 500);
                }
                if ($(".wheel-outer.w-3").length) {
                    $('html,body').animate({ scrollTop: $($wheelContainer).offset().top }, 500);
                }
            }

            var $vSlideWrap = $(".v-slide-wrap");
            $vSlideWrap.removeClass("vslide-1").removeClass("vslide-2").removeClass("vslide-3");
            var $vSlideDataItem = $vSlideWrap.children(".pages").find(".active").data("vslide-item");
            $vSlideWrap.addClass($vSlideDataItem);

            if ($(window).width() > 839) {
                if ($(".v-slide-wrap.vslide-2").length) {
                    $('html,body').animate({ scrollTop: $($vSlideWrap).offset().top }, 500);
                }
            }
            $vSlideWrap.children(".pages").attr("aria-selected", false);
            $vSlideWrap.children(".pages").find(".active").attr("aria-selected", true);

            var $vSlideBtnParent = $(".v-slide").children("ul").find(".active").children("div");
            var $vSlide = $(".v-slide");
            $vSlide.find(".btn").attr("tabindex", -1);
            $vSlide.find(".arrow-right-l").attr("tabindex", -1);
            $vSlideBtnParent.find(".btn").attr("tabindex", 0);
            $vSlideBtnParent.find(".arrow-right-l").attr("tabindex", 0);
            $vSlide.children(".pages").find(".active").attr("aria-selected", true);

            /*
            var $tabScrollWrap = $(".tabscroll");
            $tabScrollWrap.removeClass("tabscroll-1").removeClass("tabscroll-2").removeClass("tabscroll-3");
            var $tabScrollDataItem = $tabScrollWrap.siblings(".tab-pages").find(".active").data("tabscroll-item");
            $tabScrollWrap.addClass($tabScrollDataItem);

            if ($(".tabscroll.tabscroll-2").length) {
                $('html,body').animate({ scrollTop: $($tabScrollWrap).offset().top }, 500);
            }
            */
        ...
        self.slideBy = function (delta, immediate) {
            ...
            if (itemNav) {
                self[centeredNav ? 'toCenter' : 'toStart'](
                    within((centeredNav ? rel.centerItem : rel.firstItem) + o.scrollBy / 4 * delta, 0, items.length)
                );
            } else {
        ...
        function scrollHandler(event) {
            ...
            // if (delta > 0 && pos.dest == pos.start || delta < 0 && pos.dest == pos.end) {
            // if (!($(this) == $(".wheel-co-wrap"))) {
            //  $('html, body').animate({
    ...
    Sly.defaults = {
        ...
        speed: 900,       // Animations speed in milliseconds. 0 to disable animations.
