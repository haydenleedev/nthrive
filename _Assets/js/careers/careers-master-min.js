...
function videoClose() { location.reload(); } !(function (root, factory) { if (typeof define === 'function' && define.amd) { define(['jquery'], function ($) { return factory(root, $); }); } else if (typeof exports === 'object') { factory(root, require('jquery')); } else { factory(root, root.jQuery || root.Zepto); } })(this, function (global, $) {
    ...
    $("a").attr("data-focus", "none"); var $nthMenuAnchor = $('#nth-menuanchor-open'); var $notOpen = $('#primary-nav > ul > li:not(#nth-menuanchor-open)'); $('html').removeClass('no-js'); $nthMenuAnchor.on('click', function () {
    ...
    $('a').on('keyup change', function (e) { $('a').attr("data-focus", "none"); if (e.keyCode === 9 && $(this).is(":focus")) { $(this).attr("data-focus", "focused"); } }); $(document).on('keydown', function (e) {
    ...
    }); $('li:has("ul") > a').addClass('menu-hover'); $('li.nav-dropdown').hover(function () { $(this).children('ul.first').after('<span class="uparrow"></span>'); }, function () { $(this).children('ul.first').nextAll().remove(); }); $('li:has("ul")').on('mouseover keyup click mouseleave', function (e) {
...
$(document).ready(function () {
    ...
    $("#careers-question").validate({ messages: { first_name: { required: !0, minlength: 3 }, last_name: { required: !0, minlength: 3 }, reason_for_contact: { required: true }, email: { required: true }, Contact_me_Details__c: { required: !0, minlength: 5 } }, messages: { first_name: { required: "Please enter your first name", minlength: "Your first name must consist of at least 3 characters" }, last_name: { required: "Please enter your last name", minlength: "Your last name must consist of at least 3 characters" }, reason_for_contact: "Please select your reason for contact", email: { required: "Please enter a valid email address" }, Contact_me_Details__c: { required: "Please enter your question", minlength: "Your question must consist of at least 5 characters" } } }); $('#careers-question').submit(function () { $(".select-style-light").after($("#reason-for-contact-error")); }); $('#search-form :input').focus(function () { $(this).parent().addClass("valid-group").children("label.label-focus").removeClass("label-focus").addClass("label-blur"); }); $('#search-form :input').blur(function () { if ($('#jsb-keywords').val().length < 1) { $(this).prev().removeClass('label-blur').addClass('label-focus'); } else { $(this).prev().removeClass('label-focus').addClass('label-blur'); } }); $('#careers-question :input').focus(function () { $(this).parent().addClass("valid-group").children('label.label-focus').removeClass("label-focus").addClass("label-blur"); }); $('#careers-question :input').blur(function () { $(this).next('label.label-blur').removeClass("label-blur").addClass("label-focus"); }); var $secondNav = $("#secondary-nav"); var $mainNav = $(".last-line"); var $pageNav = $("#page-nav .l-site-width"); function winWidth() { if ($(window).width() < 829) { $mainNav.append($secondNav); } else { $pageNav.append($secondNav); } }
    winWidth(); $(window).resize(function () { winWidth(); }); var $box = $(".box"); var $over = ".over"; var $animWrap = ".anim-wrap"; $box.hover(function (e) {
        if ($(this).attr('aria-expanded') == 'false') { $(this).attr('aria-expanded', 'true'); $($over).attr('aria-hidden', 'false'); $(this).children($animWrap).stop(true, true).animate({ left: "237px" }, "fast"); $(this).children($over).stop(true, true).delay(100).animate({ left: "0" }, "fast"); } else { $(this).attr('aria-expanded', 'false'); $($over).attr('aria-hidden', 'true'); $(this).siblings().attr('aria-expanded', 'false'); $($over).siblings().attr('aria-hidden', 'true'); $(this).children($animWrap).stop(true, true).delay(100).animate({ left: "0" }, "fast"); $(this).children($over).stop(true, true).animate({ left: "-237px" }, "fast"); }
        e.stopPropagation(); return false;
    }); $box.on('touchstart', function (e) { 'use strict'; var link = $(this); if ($(this).attr('aria-expanded') == 'false') { e.preventDefault(); $(this).attr('aria-expanded', 'true'); $($over).attr('aria-hidden', 'false'); $(this).children($animWrap).stop(true, true).animate({ left: "237px" }, "fast"); $(this).children($over).stop(true, true).delay(100).animate({ left: "0" }, "fast"); return true; } else { $(this).attr('aria-expanded', 'false'); $($over).attr('aria-hidden', 'true'); $(this).siblings().attr('aria-expanded', 'false'); $($over).siblings().attr('aria-hidden', 'true'); $(this).children($animWrap).stop(true, true).delay(100).animate({ left: "0" }, "fast"); $(this).children($over).stop(true, true).animate({ left: "-237px" }, "fast"); return false; } }); var $animation_elements = $('.anim-pic'); var $window = $(window); function check_if_in_view() {
...
}); $(document).ready(function () { console.log("DOM is ready"); $('nav a[href^="#"]').click(function () { var target = $(this.hash); if (target.length == 0) target = $('a[name="' + this.hash.substr(1) + '"]'); if (target.length == 0) target = $('html'); $('html, body').animate({ scrollTop: target.offset().top }, 500); return false; }); $(window).on("load", function () { var tl1 = new TimelineMax({ delay: 0.3 }); tl1.to('.inner-banner .anim-pic', 0.4, { opacity: 1 }).to('.l-banner-h-width #b-4', 0.9, { bottom: 0, opacity: 1, ease: Power2.easeOut }, '0.3').to('.l-banner-h-width #b-1', 1.1, { bottom: 22, opacity: 1, ease: Power2.easeOut }, '-=0.8').to('.l-banner-h-width #b-6', 1.0, { bottom: 0, opacity: 1, ease: Power2.easeOut }, '-=1.0').to('.l-banner-h-width #b-7', 1.0, { bottom: 11, opacity: 1, ease: Power2.easeOut }, '-=1.0').to('.l-banner-h-width #b-3', 2.5, { bottom: 92, opacity: 1, ease: Power2.easeOut }, '-=2').to('.l-banner-h-width #b-3', 3.5, { bottom: 72, opacity: 1, ease: Power0.ease }).to('.l-banner-h-width #b-2', 2, { bottom: 27, opacity: 1, ease: Power2.easeOut }, '-=5.0').to('.l-banner-h-width #b-2', 3.5, { bottom: -27, ease: Power0.ease }, '-=3.5').to('.l-banner-h-width #b-5', 2.4, { bottom: 20, opacity: 1, ease: Power2.easeOut }, '-=5.0').to('.l-banner-h-width #b-5', 3.5, { bottom: -20, opacity: 1, ease: Power0.ease }, '-=3.0'); var tl11 = new TimelineMax({ delay: 0.3 }); tl11.to('.inner-banner .anim-pic', 0.4, { opacity: 1 }).to('.l-banner-v-width #b-1', 0.9, { bottom: 160, opacity: 1, ease: Power2.easeOut }, '0.3').to('.l-banner-v-width #b-2', 0.9, { bottom: 207, opacity: 1, ease: Power2.easeOut }, '-=0.6').to('.l-banner-v-width #b-3', 1.0, { bottom: 178, opacity: 1, ease: Power2.easeOut }, '-=0.5').to('.l-banner-v-width #b-4', 1.2, { bottom: 75, opacity: 1, ease: Power2.easeOut }, '-=1.8').to('.l-banner-v-width #b-4', 1.5, { bottom: 0, opacity: 1, ease: Power0.ease }, '-=1.1').to('.l-banner-v-width #b-6', 1.0, { bottom: 53, opacity: 1, ease: Power2.easeOut }, '-=1.9').to('.l-banner-v-width #b-6', 1.5, { bottom: 3, opacity: 1, ease: Power0.ease }, '-=1.0').to('.l-banner-v-width #b-5', 1.3, { bottom: 30, opacity: 1, ease: Power2.easeOut }, '-=2.7'); tl1.restart(); tl11.restart(); }); }); var iScrollPos = 0; var $bannerPic = $('.inner-banner .anim-pic'); var $anaPic = $('.alter-content .anim-pic'); $(window).scroll(function () {
    var iCurScrollPos = $(this).scrollTop(); var $document = $(document); if ((iCurScrollPos > iScrollPos)) { $bannerPic.addClass('scroll-down').removeClass('scroll-up'); var tl2 = new TimelineMax({ delay: 0.3 }); tl2.to('.l-banner-h-width #b-4', 0.9, { bottom: 100, opacity: 0, ease: Power2.easeOut }).to('.l-banner-h-width #b-1', 1.1, { bottom: 122, opacity: 0, ease: Power2.easeOut }, '-=0.5').to('.l-banner-h-width #b-6', 1.5, { bottom: 100, opacity: 0, ease: Power2.easeOut }, '-=1.2').to('.l-banner-h-width #b-7', 1.6, { bottom: 111, opacity: 0, ease: Power2.easeOut }, '-=1.6').to('.l-banner-h-width  #b-3', 2, { bottom: 192, opacity: 0, ease: Power2.easeOut }, '-=1.6').to('.l-banner-h-width #b-2', 3, { bottom: 127, opacity: 0, ease: Power2.easeOut }, '-=2.0').to('.l-banner-h-width #b-5', 4.5, { bottom: 120, opacity: 0, ease: Power2.easeOut }, '-=3.2'); var tl12 = new TimelineMax(); tl12.to('.l-banner-v-width #b-1', 0.9, { bottom: 360, opacity: 0, ease: Power2.easeOut }).to('.l-banner-v-width #b-2', 0.9, { bottom: 407, opacity: 0, ease: Power2.easeOut }, '-=0.8').to('.l-banner-v-width #b-3', 1.0, { bottom: 378, opacity: 0, ease: Power2.easeOut }, '-=0.5').to('.l-banner-v-width #b-4', 1.2, { bottom: 200, opacity: 0, ease: Power2.easeOut }, '-=1.1').to('.l-banner-v-width #b-6', 1.0, { bottom: 203, opacity: 0, ease: Power2.easeOut }, '-=0.9').to('.l-banner-v-width #b-5', 1.3, { bottom: 230, opacity: 0, ease: Power2.easeOut }, '-=0.7'); } else { $bannerPic.removeClass('scroll-down').addClass('scroll-up'); var tl3 = new TimelineMax({ delay: 0.4 }); tl3.to('.l-banner-h-width .scroll-up #b-4', 0.9, { bottom: 0, opacity: 1, ease: Power2.easeOut }).to('.l-banner-h-width .scroll-up #b-1', 1.1, { bottom: 22, opacity: 1, ease: Power2.easeOut }, '-=0.8').to('.l-banner-h-width .scroll-up #b-6', 1.0, { bottom: 0, opacity: 1, ease: Power2.easeOut }, '-=0.9').to('.l-banner-h-width .scroll-up #b-7', 1.0, { bottom: 11, opacity: 1, ease: Power2.easeOut }, '-=0.7').to('.l-banner-h-width .scroll-up #b-3', 1.5, { bottom: 72, opacity: 1, ease: Power2.easeOut }, '-=1').to('.l-banner-h-width .scroll-up #b-2', 2.4, { bottom: -27, opacity: 1, ease: Power2.easeOut }, '-=2.1').to('.l-banner-h-width .scroll-up #b-5', 3.2, { bottom: -20, opacity: 1, ease: Power2.easeOut }, '-=2.0'); var tl13 = new TimelineMax({ delay: 0.2 }); tl13.to('.l-banner-v-width #b-1', 0.9, { bottom: 160, opacity: 1, ease: Power2.easeOut }).to('.l-banner-v-width #b-5', 1.3, { bottom: 30, opacity: 1, ease: Power2.easeOut }, '-=0.5').to('.l-banner-v-width #b-2', 0.9, { bottom: 207, opacity: 1, ease: Power2.easeOut }, '-=0.8').to('.l-banner-v-width #b-3', 1.0, { bottom: 178, opacity: 1, ease: Power2.easeOut }, '-=0.9').to('.l-banner-v-width #b-4', 1.2, { bottom: 0, opacity: 1, ease: Power2.easeOut }, '-=1.1').to('.l-banner-v-width #b-6', 1.0, { bottom: 3, opacity: 1, ease: Power2.easeOut }, '-=0.9'); tl3.restart(); tl13.restart(); }
    if (iCurScrollPos > iScrollPos) { $anaPic.addClass('scroll-down').removeClass('scroll-up'); } else { $anaPic.removeClass('scroll-down').addClass('scroll-up'); }
