$(document).ready(function () {
    ...
    var $subscribeFormInput = $('#subscribe-form :input');
    var $subscribeFormBody = $("#subscribe-form-body");
    var $subscribeFormInput = $("#subscribe-form-body :input");
    var $thankYouBody = $("#thank-you-body");
    ...
    if (window.location.href.indexOf("b309b5f2-3dd0-40b1-92c0-c0c605ff664e") > -1) {
    ...
    if (window.location.href.indexOf("WcoFormId=e03e8096-9bb4-4cf4-9006-1d86466b63a2") > -1) {
        $subscribeFormBody.addClass("hide");
        $thankYouBody.removeClass("hide");
        $("html, body").animate({
            scrollTop: $("#subscribe-body-anchor").offset().top - 150
        }, 'fast');
    }
    $subscribeFormBody.submit(function () {
        $.ajax({
            type: 'POST',
            url: 'http://info.nthrive.com/l/311601/2017-06-14/d34n',
            data: {
...
function videoClose() {
    ...
    function Remodal($modal, options) {
        ...
        popup.$modal = $modal.addClass(NAMESPACE + ' ' +
            namespacify('is-initialized') + ' ' +
            popup.settings.modifier + ' ' +
            namespacify('is', STATES.CLOSED)).attr('tabindex', '-1');
        popup.$wrapper = $('<div>').addClass(namespacify('wrapper') + ' ' +
            popup.settings.modifier + ' ' +
            namespacify('is', STATES.CLOSED)).hide().append(popup.$modal);
...
$(document).ready(function () {
    $(':input', 'form').not(':button, :submit, :reset, :hidden, :checkbox').val('').prop('checked', false).prop('selected', false);
});
//----------- START script ---------------------------
$(document).ready(function () {
    $("a").attr("data-focus", "none");
    $("button").attr("data-focus", "none");
    $("a").removeClass("focus-h");
    var $nthMenuAnchor = $('#nth-menuanchor-open');
    var $notOpen = $('#primary-nav > ul > li:not(#nth-menuanchor-open)');
    // Remove no-js class
    $('html').removeClass('no-js');
    $nthMenuAnchor.on('click', function () {
        if ($(this).hasClass('nth-open')) {
            $notOpen.removeClass('nth-show-menu');
            $(this).removeClass('nth-open');
            $(this).attr('aria-expanded', false);
        } else {
            $notOpen.addClass('nth-show-menu');
            $(this).addClass('nth-open');
    ...
    $('a').on('keyup change', function (e) {
        ...
        if (e.keyCode === 9 && $(this).is(":focus")) {
            ...
            $(this).addClass("focus-h");
        }
        if (e.shiftKey && $(this).is(":focus")) {
            $(this).attr("data-focus", "focused");
            $(this).removeClass("focus-h");
        }
    });
    ...
    $(document).on('keydown', function (e) {
        if (e.keyCode === 27) {
            $notOpen.removeClass('nth-show-menu');
            $nthMenuAnchor.removeClass('nth-open');
            $nthMenuAnchor.attr('aria-expanded', false);
        }
    ...
    $('li:has("ul") > a').addClass('menu-hover');
    // First dropdown
    ...
    $('li:not(:has(ul))').on('mouseover keyup click mouseleave', function (e) {
        if (e.keyCode === 16 | e.keyCode === 9 | e.type === 'mouseover') {
            $(".nav-dropdown").children("ul").removeClass('nth-show-menu').addClass('nth-hide-menu');
            $(".menu-hover").attr('aria-expanded', false);
        }
    });
    // sub menu
    // ------------------------
    ...
    $('li:has("ul")').on('mouseover keyup click mouseleave', function (e) {
        ...
        if (e.keyCode === 9 | e.type === 'mouseover') {
            // Show sub menu
            $(this).children('ul').removeClass('nth-hide-menu').addClass('nth-show-menu');
            $(this).children('a').attr('aria-expanded', true);
        }
        // If mouse leaves li that has sub menu
        if (e.type === 'mouseleave') {
            // hide sub menu
            $(this).children('ul').removeClass('nth-show-menu').addClass('nth-hide-menu');
            $(this).children('a').attr('aria-expanded', false);
        }
        // If clicking on li that has a sub menu
        if (e.type === 'click') {
            // If sub menu is already open
            if ($(this).children('a').hasClass('nth-openSubMenu')) {
                // remove Open class
                $(this).children('a').removeClass('nth-openSubMenu').attr('aria-expanded', false);
                // Hide sub menu
                $(this).children('ul').removeClass('nth-show-menu').addClass('nth-hide-menu');
                // If sub menu is closed
            } else {
                // add Open class
                $(this).children('a').addClass('nth-openSubMenu').attr('aria-expanded', true);
                // Show sub menu
                $(this).children('ul').removeClass('nth-hide-menu').addClass('nth-show-menu');
            }
            return true;
    ...
    $('li > ul > li:last-child > a').on('keydown', function (e) {
        ...
        if ((e.keyCode == 9) && $(this).parent('li').children('ul').length == 0) {
            // Close this sub menu
            $(this).parent('li').parent('ul').removeClass('nth-show-menu').addClass('nth-hide-menu');
            // If tabbing out of a third level sub menu and there are no other links in the parent (level 2) sub menu
            $(this).parent('li').parent('ul').prev('a').attr('aria-expanded', false);
            if ($(this).parent('li').parent('ul').parent('li').parent('ul').parent('li').children('ul').length > 0
                && $(this).parent('li').parent('ul').parent('li').is(':last-child')) {
                // Close the parent sub menu (level 2) as well
                $(this).parent('li').parent('ul').parent('li').parent('ul').removeClass('nth-show-menu').addClass('nth-hide-menu');
                $(this).parent('li').parent('ul').prev('a').attr('aria-expanded', true);
            }
        }
    })
})
...
$(document).ready(function () {
    var catLocation = $(this).val();
    $('#jsb-position').change(function () {
        var catFilter = $('#jsb-position option:selected').text();
        if ($('#jsb-position').val().length > 0) {
            $('#filter-wrap').removeClass('hide1');
            $('#filter-1').remove();
            $('#filter-wrap').append('<div class="filter-list l-align-left" id="filter-1"><a href="#" class="filter-item" id="filter-item-1">' + catFilter + '</a></div>');
        } else {
            $('#filter-wrap').addClass('hide1');
            $('#filter-1').remove();
        }
    });
    $('#jsb-location').change(function () {
        var catFilter = $('#jsb-location option:selected').text();
        if ($('#jsb-location').val().length > 0) {
            $('#filter-wrap').removeClass('hide2');
            $('#filter-2').remove();
            $('#filter-wrap').append('<div class="filter-list l-align-left" id="filter-2"><a href="#" class="filter-item" id="filter-item-2">' + catFilter + '</a></div>');
        } else {
            $('#filter-wrap').addClass('hide2');
            $('#filter-2').remove();
        }
    });
    ...
    $('#clear-filters').click(function () {
        $('#filter-1').remove();
        $('#filter-2').remove();
        $('#filter-wrap').addClass('hide1').addClass('hide2');
        $('#jsb-position').val('').trigger('change');
        $('#jsb-location').val('').trigger('change');
    });
...
$(document).ready(function () {
    ...
    $('#search-form :input').blur(function () {
        if ($('#jsb-keywords').val().length < 1) {
            $(this).prev().removeClass('label-blur').addClass('label-focus');
        } else {
            $(this).prev().removeClass('label-focus').addClass('label-blur');
        }
    });
    ...
    var $mainNav = $(".last-line");
    var $pageNav = $("#page-nav .l-site-width");
    function winWidth() {
        if ($(window).width() < 829) {
            $mainNav.append($secondNav);
        } else {
    ...
    var $box = $(".box");
    var $over = ".over";
    var $animWrap = ".anim-wrap";

    $box.each(function () {
        $box.hover(function () {
            $(this).attr('aria-expanded', 'true');
            $($over).attr('aria-hidden', 'false');
        }, function () {
            $(this).attr('aria-expanded', 'false');
            $($over).attr('aria-hidden', 'true');
            $(this).siblings().attr('aria-expanded', 'false');
            $($over).siblings().attr('aria-hidden', 'true');
        });
    });

    $box.on('keyup change', function (e) {
    ...
    /*** Image Animation ***/
    var $animation_elements = $('.anim-pic');
    var $window = $(window);
...
$(document).ready(function () {
    ...
    $(window).on("load", function () {
        var tl1 = new TimelineMax({ delay: 0.3 });
        tl1.to('.inner-banner .anim-pic', 0.4, { opacity: 1 })
            .to('.l-banner-h-width #b-4', 0.9, { bottom: 0, opacity: 1, ease: Power2.easeOut }, '0.3')
            .to('.l-banner-h-width #b-1', 1.1, { bottom: 22, opacity: 1, ease: Power2.easeOut }, '-=0.8')
            .to('.l-banner-h-width #b-6', 1.0, { bottom: 0, opacity: 1, ease: Power2.easeOut }, '-=1.0')
            .to('.l-banner-h-width #b-7', 1.0, { bottom: 11, opacity: 1, ease: Power2.easeOut }, '-=1.0')
            .to('.l-banner-h-width #b-3', 2.5, { bottom: 92, opacity: 1, ease: Power2.easeOut }, '-=2').to('.l-banner-h-width #b-3', 3.5, { bottom: 72, opacity: 1, ease: Power0.ease })
            .to('.l-banner-h-width #b-2', 2, { bottom: 7, opacity: 1, ease: Power2.easeOut }, '-=5.0')
            .to('.l-banner-h-width #b-5', 2.4, { bottom: 20, opacity: 1, ease: Power2.easeOut }, '-=5.0');

        var tl11 = new TimelineMax({ delay: 0.3 });
        tl11.to('.inner-banner .anim-pic', 0.4, { opacity: 1 })
            .to('.l-banner-v-width #b-1', 0.9, { bottom: 160, opacity: 1, ease: Power2.easeOut }, '0.3')
            .to('.l-banner-v-width #b-2', 0.9, { bottom: 207, opacity: 1, ease: Power2.easeOut }, '-=0.6')
            .to('.l-banner-v-width #b-3', 1.0, { bottom: 178, opacity: 1, ease: Power2.easeOut }, '-=0.5')
            .to('.l-banner-v-width #b-4', 1.2, { bottom: 75, opacity: 1, ease: Power2.easeOut }, '-=1.8').to('.l-banner-v-width #b-4', 1.5, { bottom: 0, opacity: 1, ease: Power0.ease }, '-=1.1')
            .to('.l-banner-v-width #b-6', 1.0, { bottom: 53, opacity: 1, ease: Power2.easeOut }, '-=1.9').to('.l-banner-v-width #b-6', 1.5, { bottom: 3, opacity: 1, ease: Power0.ease }, '-=1.0')
            .to('.l-banner-v-width #b-5', 1.3, { bottom: 30, opacity: 1, ease: Power2.easeOut }, '-=2.7');

        tl1.restart();//Go back to the start (true is to suppress events)
        tl11.restart(); //Go back to the start (true is to suppress events)
    });
...
var iScrollPos = 0;
var $bannerPic = $('.inner-banner .anim-pic');
var $anaPic = $('.alter-content .anim-pic');
$(window).scroll(function () {
    ...
    if (iCurScrollPos > iScrollPos) {
        //Scrolling Down
        $anaPic.addClass('scroll-down').removeClass('scroll-up');
    } else {
        $anaPic.removeClass('scroll-down').addClass('scroll-up');
    }
...
$('.panel-title').each(function (i) {
    ...
    $target = $(this).parent('.acc-header-wrap').next('.panel-content')[0].id;
...
$('.cta-btn .panel-title ul').append('<li class="arrow-row"><span class="arrow-toggle"><span class="t-circle t-toggle-circle"></span><span class="t-arrow-down t-toggle-arrow-down"></span></span></li>');
// Optional : include an icon. Better in JS because without JS it have non-sense.
$('.no-cta-btn .panel-title a').after('<span class="t-circle t-toggle-circle"></span><span class="t-arrow-down t-toggle-arrow-down"></span>');
...
$('.panel-title a').click(function () {
    if ($(this).attr('aria-expanded') == 'false') { //If aria expanded is false then it's not opened and we want it opened !
        ...
        $(this).parents('.accordion').find('[aria-expanded=true]').attr('aria-expanded', false).removeClass('active').parent().parent('.acc-header-wrap').next('.panel-content').slideUp(200).attr('aria-hidden', 'true');
        ...
        $(this).attr('aria-expanded', true).addClass('active').parent().parent('.acc-header-wrap').next('.panel-content').slideDown(200).attr('aria-hidden', 'false');
    } else { // The current panel is opened and we want to close it
        $(this).attr('aria-expanded', false).removeClass('active').parent().parent('.acc-header-wrap').next('.panel-content').slideUp(200).attr('aria-hidden', 'true');;
    }
    // No Boing Boing
    return false;
});